        ...
        key = InlineKeyboardMarkup(
            [
                ...
                [
                    ...
                    InlineKeyboardButton(text="Tutup 🗑️", callback_data=f"close2"),
                ],
            ]
        )
    elif j == 6:
...
async def ytdata(_, CallbackQuery):
    ...
    if type == "audio":
        a1 = InlineKeyboardButton(
            text=f"Bentuk Audio", callback_data=f"boom audio||{format}||{videoid}"
        )
        a2 = InlineKeyboardButton(
            text=f"Bentuk Dokumen", callback_data=f"boom docaudio||{format}||{videoid}"
        )
    else:
        a1 = InlineKeyboardButton(
            text=f"Bentuk Video", callback_data=f"boom video||{format}||{videoid}"
        )
        a2 = InlineKeyboardButton(
            text=f"Bentuk Dokumen", callback_data=f"boom docvideo||{format}||{videoid}"
        )
    key = InlineKeyboardMarkup(
        [
            [
...
def p_mark(link, channel):
    buttons = [
        ...
        [InlineKeyboardButton(text="Kunjungi Channel Youtube", url=f"{channel}")],
    ]
    return buttons
